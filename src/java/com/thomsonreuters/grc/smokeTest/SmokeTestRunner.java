...
/**
 * <p>Runs the smoke test methods found in a provided class and collects and processes their results.</p>
 *
 *
 * <p>Classes provided to the {@link #runTests} method should itself have methods that
 * and annotate those with then {@link SmokeTest} annotation.  Smoke test methods should
 * not require any parameters, and should return a {@link SmokeTestResult} object.</p>
 *
 * <p>The smoke test class can also provide a setter for an {@link AppTestContext} class.  If present,
 * it will be called with the context class for the current test session.</p>
 *
 * <p>Smoke test classes can optionally be annotated with {@link ComponentSmokeTest} or
 * {@link IntegrationSmokeTest} to indicate the type of smoke tests that a class contains,
 * however, this is not required by this class.</p>
 * For example:
 * <pre>
...
public class SmokeTestRunner {
     ...
     * 
     * @param smokeTestClass the class whose smoke tests will be invoked.
    ...
    /**
     * Check if the given class has a setter for the AppTestContext, and if so, call it to pass
     * the smoke test context to it.
     * 
     * @param smokeTestClass the class whose smoke tests will be invoked.
     */
     ...
     * 
     * @param smokeTestClass    The class whose smoke test method will be called.
     * @param method        The smoke test method.
     * @param testName      The test name to use for this method.
     * @return The result from the smoke test execution.
     */
    protected AppTestResult runSmokeTestMethod(Object smokeTestClass,
                                               Method method,
                                               String testName) {
        try {
            // Run the test
            ...
            String comment = "The test threw the exception " + exceptionName
            ...
            String comment =
                    "Unable to run the test; Exception: " + exceptionName + "; message: ["
                            + message + "]";
            return createAppTestResult(testName, SmokeTestResult.failed(comment));
        }
    }
     ...
     */
    protected AppTestResult createAppTestResult(String testName,
    ...
    /** @param appTestContext The context values provided by the caller of this object. */
