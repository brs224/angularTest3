 ...
 * 
 * Extend this class for simple smoke tests that would exercise a GET command on
 * a given URL and then read the response as a stream.
 */
public abstract class WebResourceSmokeTest {
    ...
    /**
     * @return A URL including the local server name and port. Ex:
     *         http://ctjdg9084:8183. This method is to be used when running
     *         component smoke tests as it would not depend on BigIP being
     *         available for mapping resolution.
     * 
     * @throws Exception
     */
    ...
    /**
     * @return A URL including the server name and port specified in the
     *         request. Ex: http://qa.workbench.judicial.int.westgroup.com:80.
     *         This method is bo be used when running integration smoke tests as
     *         BigIP will be required to perform mapping resolution.
     * 
     * @throws Exception
     */
    protected final String getRequestDomainURL() throws Exception {

        HttpServletRequest request = (HttpServletRequest) appTestContext
                .getObjectValue(SmokeTestUtils.TEST_CTX_REQUEST);

        return "http://" + request.getServerName() + ":" + request.getServerPort();
    }

    /**
     * Performs a smoke test by reading the response from a GET on the specified
     * resource.
     * 
     * @param resourceName
     *            The name of the resource to be used for messages produced by
     *            the test.
     * @param resourceURI
     *            The base URI to the resource, usually the domain URL.
     * @param additionalPath
     *            Any additional path to be appended to the resource URI.
