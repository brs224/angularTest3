...
/**
 * Overrides WebTestCommand to pass additional information in
 * <code>AppTestContext</code> to the tests being run.
 */
public class WebServicesTestCommand extends WebTestCommand {
     ...
     * 
     * @param appTestName
     *            The name used for this test run.
     * @param releaseLabel
     *            The version retrieved from the manifest, or provided by the
     *            build tool.
     * @param context
     *            The servlet context this test will run in.
     */
     ...
     *             when a servlet exception is encountered.
     * @throws IOException
        ...
        try {
            ...
            if (outputFormat == OUTPUT_HTML) {
                ...
                formatter.format(results, appTestImpl.getApplicationName(), appTestImpl.getVersion(), manifestVersion,
                        appTestImpl.getEnvironment(), appTestImpl.getServer(), new CustomPrintWriter(out, testType),
                        testType);
    ...
    /**
     * @return The version retrieved from the manifest, or provided by the build
     *         tool.
     */
    ...
    /**
     * Dynamically create the class identified in the appTestImplClassName and
     * insure it supports the the AppTest interface.
     * 
     * @throws ServletException
     *             in the event of a ClassNotFoundException caught when trying
     *             to create the Class.
     */
    ...
    /**
     * Used to dynamically create an instance of a desired class and insure it
     * supports the desiredInterfaceName.
     * 
     * @param className
     *            concrete type of requested class
     * @param desiredInterfaceName
     *            to cast into
     * @return the requested class as an object which can be casted to the
     *         desiredInterfaceName
     * @throws ClassNotFoundException
     *             in the event of any exception being caught including if the
     *             class could not be found and if the class does not support
    ...
    private static Object createObject(String className, String desiredInterfaceName) throws ClassNotFoundException {
        ...
        try {
            ...
            // may as well test so we can log if the interface is not
            // implemented!
    ...
    /**
     * @return The server name and port specified in the request. Ex:
     *         qa.workbench.judicial.int.westgroup.com:80.
     */
    ...
    protected String formatXmlOutput(AppTestResult[] testResults, AppTestContext context) {
        /*
         * StringBuilder result = new StringBuilder("<results>"); for
         * (AppTestResult testResult : testResults) { result.append("<test>");
         * result.append("<name>").append(testResult.getTestName()).append(
         * "</name>"); if (testResult.isPassed()) {
         * result.append("<result>passed</name>"); } else if
         * (testResult.isFatal()) { result.append("<result>fatal</name>"); }
         * else { result.append("<result>failed</name>"); }
         * result.append("</test>"); }
         * 
    ...
    /**
     * Used to inject a link before the HTML is closed; works around the private
     * methods of <code>WebTestHtmlResponseFormatter</code>.
     */
    private static final class CustomPrintWriter extends PrintWriter {
        ...
        /**
         * Create a custom print writer that sends all requests to the orig
         * PrintWriter, and knows which report is being generated.
         */
        ...
        /**
         * If the HTML is being closed, insert links to the other smoke test
         * page.
         * 
         * @param s
         */
