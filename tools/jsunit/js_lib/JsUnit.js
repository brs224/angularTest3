...
function ComparisonFailure(msg, expected, actual, stack) {
	AssertionFailedError.call(this, (msg ? msg + " " : "") + "expected", stack);
 ...
 */
function TestFailure_failedTest() {
	return this.mTest;
}
/**
 ...
 */
function TestFailure_thrownException() {
 ...
 * @type Number
 */
function TestResult_errorCount() {
 ...
 * @type Number
 */
function TestResult_failureCount() {
 ...
 * @type Boolean
 */
function TestResult_shouldStop() {
	return this.mStop;
}
/**
 * A test starts.
...
function Assert_assertNotSame(msg, expected, actual) {
	...
	if (expected === actual)
		this.fail("Not the same expected:<" + expected + ">", new CallStack(),
				msg);
}
/**
...
function Assert_assertNotUndefined(msg, object) {
	...
	if (object === undefined)
		this.fail("Object <" + object + "> was undefined.", new CallStack(),
				msg);
}
/**
...
function Assert_assertNull(msg, object) {
	...
	if (object !== null)
		this
				.fail("Object <" + object + "> was not null.", new CallStack(),
						msg);
}
/**
...
function Assert_fail(msg, stack, usermsg) {
	var afe = new AssertionFailedError((usermsg ? usermsg + " " : "") + msg,
			stack);
	throw afe;
}
Assert.glue();
 ...
 */
function TestCase_countTestCases() {
	return 1;
}
/**
 ...
 */
function TestCase_createResult() {
 ...
 */
function TestCase_setName(name) {
	this.mName = name;
}
/**
	...
	case "object":
		if (obj !== null) {
			...
			if (typeof (obj) != "function")
				this.addTest(this.warning("Cannot instantiate test class for "
						+ "object '" + obj + "'"));
		}
		// fall through
	case "undefined":
...
function TestSuite_run(result) {
	...
	if (i == 0) {
		var ex = new AssertionFailedError("Test suite with no tests.",
				new CallStack());
 ...
 */
function TestDecorator_basicRun(result) {
 ...
 * @type Number.
 */
function TestDecorator_countTestCases() {
 ...
 * @type Test
 */
function TestDecorator_findTest(name) {
 ...
 * @type String
 */
function TestDecorator_getName() {
 ...
 * @type Test
 */
function TestDecorator_getTest() {
	return this.mTest;
}
/**
 * Run the test.
 * @tparam TestResult result The test result.
 */
function TestDecorator_run(result) {
 ...
 */
function TestDecorator_setName(name) {
 ...
 * @type String
 */
function TestDecorator_toString() {
...
function ExceptionTestCase_runTest() {
	try {
		...
		if (ex instanceof this.mClass && !(ex instanceof AssertionFailedError))
			return;
		else
			throw ex;
	}
...
function BaseTestRunner_addError(test, except) {
	this.testFailed(TestRunListener.prototype.STATUS_ERROR, test.toString(),
			except.toString());
}
/**
 * Implementation of TestListener.
...
function TestRunner_run(name, result) {
	...
	if (test == null) {
		var ex = new AssertionFailedError("Test \"" + name + "\" not found.",
				new CallStack());
...
function ResultPrinter_printFooter(result) {
	...
	if (result.wasSuccessful()) {
		...
		writer.println("Tests run: " + result.runCount() + ", Failures: "
...
function TextTestRunner_main(args) {
	...
	try {
		...
		JsUtil.prototype
				.quit(result.wasSuccessful() ? TextTestRunner.prototype.SUCCESS_EXIT
...
function TextTestRunner_start(args) {
	...
	for (var i = 0; i < args.length; ++i) {
		...
		if (optionsPossible && args[i].match(/^-/)) {
			...
			case "--xml":
				this
						.setPrinter(new XMLResultPrinter(this.mPrinter
								.getWriter()));
				continue;

			case "--classic":
				this.setPrinter(new ClassicResultPrinter(this.mPrinter
						.getWriter()));
				continue;

			case "--html":
				this.mPrinter.setWriter(new HTMLWriterFilter(this.mPrinter
						.getWriter()));
				continue;
...
function ClassicResultPrinter_addError(test, except) {
	...
	if (except.description) {
		...
		str += " - fileName: " + except.fileName + " - lineNumber: "
				+ except.lineNumber
	} else {
		str = except;
		str += " - fileName: " + except.fileName + " - lineNumber: "
...
function ClassicResultPrinter_endTest(test) {
	if (test instanceof TestSuite) {
		...
		this.writeLn("<" + this.mNest.replace(/-/g, "=")
...
function ClassicResultPrinter_printHeader(test) {
	...
	this.mInReport = true;
	this.mNest = "";
	this.writeLn("TestRunner (" + test.countTestCases()
...
function ClassicResultPrinter_printFooter(result, runTime) {
	if (result.wasSuccessful() == 0) {
		...
		this.writeLn(result.errorCount() + error + ", " + result.failureCount()
				+ failure + ".");
	} else
		this.writeLn(result.runCount() + " tests successful in "
...
function ClassicResultPrinter_startTest(test) {
	...
	if (!(test instanceof TestSuite)) {
		++this.mRunTests;
		this.writeLn(this.mNest + " Running test " + this.mRunTests + ": \""
				+ test + "\"");
	} else {
		this.writeLn(this.mNest.replace(/-/g, "=") + "> Starting test suite \""
				+ test.getName() + "\"");
		this.mNest += "-";
	}
}
...
function XMLResultPrinter_endTest(test) {
	if (this.mCurrentTest != null) {
		var endTime = new Date();
		this.mCurrentTest.mTime = this.elapsedTimeAsString(endTime
				- this.mCurrentTest.mTime);
...
function HTMLTestRunner_setPrinter(outdev) {
	...
	this.mPrinter.setWriter(new HTMLWriterFilter(this.mPrinter.getWriter()));
}
...
function AllTestsCollector_collectTests() {
	...
	if (typeof (testFunc) == "function"
			&& typeof (testFunc.prototype.suite) == "function")
		tests.push(testFunc);
	return tests;
}
...
function TestCaseCollector(scope, pattern) {
	GenericTestCollector.call(this, scope,
			typeof (pattern) == "undefined" ? /.+Test$/ : pattern, TestCase);
}
...
function EmbeddedTextTestRunner_run(testNames, suiteName) {
	...
	if (tests.length == 0) {
		...
		test = new TestSuite(typeof (suiteName) != "string" ? "TestCollection"
				: suiteName);
